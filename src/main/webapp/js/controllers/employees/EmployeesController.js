function EmployeesController($scope, $location,$route,EmployeesService,UtilityService, $timeout,$state, $log,ngTableParams ,$rootScope,ngDialog) {
	...
	$scope.edit = false ;
	$scope.addArea = false;
	$scope.heading ="Mitarbeiter hinzufügen";
	$scope.showAddArea = function(){
	   ...
	   $scope.save = function(){
		   ...
		   EmployeesService.create($scope.info).$promise.then(function (data) {
	             if (data != undefined && data != null)
	                 if (data.errorCode != undefined && data.errorCode != null)
	                 	...
	                 	UtilityService.notificationDilog("Mitarbeiter erfolgreich hinzugefügt!");
	                 	$scope.resetForm(); 
	                 }
	    ...
	    $scope.detail= function(item){
	    	...
	    	$scope.edit =  true;
	    	$scope.addArea = true;
	    	$scope.heading ="Mitarbeiter bearbeiten";
	    };
	    	...
	    	EmployeesService.update($scope.info).$promise.then(function (data) {
	             if (data != undefined && data != null)
	                 if (data.errorCode != undefined && data.errorCode != null)
	                 	...
	                 	UtilityService.notificationDilog("Mitarbeiter erfolgreich geändert!");
	                 	$scope.resetForm(); 
	    	...
	    	EmployeesService.delete({id: id}).$promise.then(function (data) {
	            if (data != undefined && data != null)
	                if (data.errorCode != undefined && data.errorCode != null)
	                	...
	                	UtilityService.notificationDilog("Der ausgewählte Mitarbeiter wurde gelöscht!");
	...
	$scope.dialogResetPass = function(item){
		...
		ngDialog.open({
	        template: 'partials/employees/resetPass.html',
    ...
    $scope.changePassword = function() {
    	EmployeesService.changePassEmployees($scope.passwordInput).$promise.then(function(data) {
        	...
        	$rootScope.error = "Das Mitarbeiter konnte nicht gelöscht werden.";
            if (error.errorCode != undefined && error.errorCode != null)
                $rootScope.error = error.errorMessage;
        });
    }
