 ...
 */
app.directive('ngTable', ['$q', '$parse','localize','$http','$cookieStore',
    function($q, $parse,localize,$http,$cookieStore) {
        'use strict';
        ...
        return {
            ...
            compile: function(element) {
            	...
            	var localizeThienbinh = $cookieStore.get('localize_blupassion');	
            	// set the default language
            	if($scope.localizeThienbinh){
            		 currentLang = localizeThienbinh;
//            		$scope.setLang($scope.currentLang);
            	}
            	else{
            		var language = window.navigator.userLanguage || window.navigator.language;
            		if(language && language =='de')
            			var currentLang = $scope.languages[0];
            		elsar
            			var currentLang =$scope.languages[1];
            	}
				...
				.success(function(data) {
	                ...
	                angular.forEach(row.find('td'), function(item) {
	                    ...
	                    titleExpr = localize.currentLocaleData[titleExpr];
	                    if (titleExpr){
